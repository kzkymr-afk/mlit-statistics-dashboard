...
旧称「国交省統計システム」。リポジトリ名・公開URLは従来どおり
`mlit-statistics-dashboard` を使用しています。

国土交通省・日建連の建設統計とBuildBaseの会社別確定値を、Excelの行列ではなく
「統計項目・分類条件・期間」で選び、表・グラフ・CSVへ出力するための
データ基盤兼ウェブアプリです。

## 現在の状態

2026年9月24日時点で、e-Stat DB/APIを主系、APIにDB表がない統計の
公式Excelを補完系として、2013年・2013年度以降（建設投資見通しは
2008年度以降）を正規化SQLiteへ格納済みです。

- データセット: 18（国交省・日建連13、労働・人材4、BuildBase会社別1）
- 統計表: 236表
- 分類の組み合わせ: 17,805,597系列
- 観測セル: 185,975,755件
- 正規化SQLite: 約29GB
- GitHub Pages用の圧縮データ: 約511MB

加えて、BuildBaseで確定したゼネコン21社・62項目・2015～2025年度の
会社別データを同じ表・グラフ・CSVで比較できます。BuildBase側に未処理セルが
ある場合は同期を停止し、非開示と公表待ちは0に変換せず状態として保持します。

公表された数値の0は、年度マスクと組み合わせて欠測と区別しながら
暗黙保持します。このため、SQLiteの観測値行は非0・欠測・秘匿・
注記付きの44,329,256行に抑え、画面では元の公表値を復元します。
...
画面全体にログインゲートがあります（許可された利用者のみ）。資格情報は
平文を置かずsalt付きSHA-256ハッシュで `lib/atlas-auth.mjs` に登録し、
セッションは端末のlocalStorageに7日間保持します。静的サイトのため
これは画面のゲートであり、公開データファイル自体のサーバ側アクセス制御では
ありません（必要になればCloudflare Access等を検討）。
...
2026年8月に3つの操作機能を追加しました。(1) 分類条件の1軸を
「複数選択で比較」に切り替えると、値をチェックして最大12系列を一括追加
できます（例: 事務所・庁舎の受注高×会社12社が3操作）。(2) データが
存在しない・非開示・公表待ちの選択肢はグレー表示と状態ラベルで区別します
（判定インデックスは `tables/{tableId}/availability.json.gz`）。
(3) 左カラムの「プリセット」に、アニュアルレポート作成で使う定型5本
（用途別受注・会社比較／大手50社受注高・用途別／21社建築受注高比較／
建設工事費デフレーター／建築着工床面積・用途別）を同梱しています
（定義は `lib/statistics-presets.mjs`）。
...
```text
/Volumes/SSD_External/Business/Platforms/mlit-statistics-dashboard
```
...
- 建設投資見通し（全国・地域別の長期時系列。2026年度見通しを主表、2025年度見通しを改訂比較用に保持、2008年度以降）
...
- 労働力調査（e-Stat DB 2表: 産業×職業別就業者、年齢階級×産業別就業者、2013年以降）
- 建設技能者（建設業の生産工程＋輸送・機械運転＋建設・採掘の3職業大分類合計。
  e-Stat DB値から計算した派生系列）と、年報I-B-5の建設業年齢構成比（2025年）
- 毎月勤労統計調査（建設業の賃金・労働時間・常用雇用の年平均、2020～2025年、公表Excel）
- 賃金構造基本統計調査（建設関連9職種の賃金・労働時間、2025年調査、公表Excel）
- 公共工事設計労務単価（全国全職種平均値、2019～2026年3月適用、国交省公表資料）

毎月勤労統計と賃金構造基本統計のe-Stat DB表は最新年を含まない（それぞれ2021年10月更新停止、
2023年まで）ため、公表Excelから抽出した値を `data/catalogs/construction-workforce.json` に
置き、`npm run sync:construction-workforce` で正規化DBへ登録します。台帳は抽出済みの
データセットから `npm run catalog:construction-workforce -- --source <フォルダ>` で作ります。
建設労働需給調査の職種別過不足率は既存の「建設労働需給調査」に収録済みです。

母集団・比較上の注意（技能者数ではない、事業所5人以上、産業計、積算単価など）は
`data/catalogs/table-notes.json` に表と分類値ごとに持ち、公開メタ情報・画面の系列欄
（注記）・CSV・AI用JSON・MCPの表スキーマに載せます。
...
- 周期: 月次・四半期・年次・年度次
- 時点: 2013年・2013年度以降（建設投資見通しのみ2008年度以降。起点はデータセットごとに `datasets.fiscal_year_from` で持つ）
